...
varying vec2 VRY_VEC2_ST;  // fragment position, unnormalized

uniform sampler2D UN_SAMP_KDTREE;
...
void getNode(const int n_idx, inout Node node){
  ...
  ivec4 lx_xx_dy_yy = ivec4(texture2D(UN_SAMP_KDTREE, st).abgr * 255.0); // .abgr !!!
...
//------------------------------------------------------------------------------
void main(void){
  NN nn = NN(VRY_VEC2_ST, 50000000.0); // init nearest neighbor values
  getNearestNeighbor(nn);              // kd-tree nearest neighbor search

  gl_FragColor.r = nn.dis;
}
